...
instance : Finite (ModuleTypeCardLT R N) := by delta ModuleTypeCardLT; infer_instance
...
instance : Finite (AlgebraTypeCardLT R N) := by delta AlgebraTypeCardLT; infer_instance
...
variable {R : Type*} [CommRing R] [TopologicalSpace R] [TopologicalRing R] [Algebra.TopologicallyFG ℤ R]
...
instance : Finite (TopologicalModuleTypeCardLT R N) := by
...
variable (R) in
def TopologicalAlgebraTypeCardLT [TopologicalRing R] [Algebra.TopologicallyFG ℤ R] (N : ℕ) : Type _ :=
  Σ' (n : Fin N) (_ : Ring (Fin n)) (_ : TopologicalSpace (Fin n)) (_ : T2Space (Fin n))
    (_ : Algebra R (Fin n)), ContinuousSMul R (Fin n)

instance : Finite (TopologicalAlgebraTypeCardLT R N) := by
  apply (config := { allowSynthFailures := true }) Finite.instPSigma; intro
  apply (config := { allowSynthFailures := true }) Finite.instPSigma; intro
  apply (config := { allowSynthFailures := true }) Finite.instPSigma; intro
  apply (config := { allowSynthFailures := true }) Finite.instPSigma; intro
