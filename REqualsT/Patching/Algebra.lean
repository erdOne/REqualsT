...
variable [TopologicalSpace Λ] [TopologicalRing Λ] [∀ i, ContinuousSMul Λ (R i)]
...
instance (k) : Finite (PatchingAlgebra.Component R F k) :=
...
omit [∀ i, TopologicalSpace (R i)] [∀ i, TopologicalRing (R i)]
...
instance : TopologicalRing (PatchingAlgebra R F) :=
  inferInstanceAs (TopologicalRing (PatchingAlgebra.subring R F))
...
variable [TopologicalRing Rₒₒ] [Algebra.TopologicallyFG ℤ Rₒₒ]
...
-- omit [TopologicalRing R] [CompactSpace R] in
omit
  [∀ (i : ι), TopologicalSpace (R i)]
  [∀ (i : ι), TopologicalRing (R i)]
  [∀ (i : ι), CompactSpace (R i)]
...
lemma RingHom.continuous_of_finite_of_compact {R H : Type*} [CommRing R] [Semiring H]
    ...
    [TopologicalRing R] [T2Space R] [IsNoetherianRing R] [ContinuousAdd H] :
    Continuous f := by
...
lemma PatchingAlgebra.map_surjective
  ...
  obtain ⟨v, hv⟩ := nonempty_inverseLimit_of_finite (ι := ℕᵒᵈ) (s ·) fs (by
      ...
      simp only [Pi.ringHom_apply, RingHom.coe_comp, Function.comp_apply, Pi.evalRingHom_apply,
         UltraProduct.mapRingHom_π, UltraProduct.π_eq_iff, id_eq]
      filter_upwards with k
...
lemma PatchingAlgebra.algebraMap_continuous
    (R : Type*) [CommRing R] [IsLocalRing R] [TopologicalSpace R] [TopologicalRing R]
