...
instance : Algebra (Π i, R i) (UltraProduct A F) where
  ...
  __ := (UltraProduct.π A F).comp (algebraMap _ _)
...
instance : TopologicalAddGroup (UltraProduct M F) where
instance : TopologicalRing (UltraProduct R F) where
...
lemma UltraProduct.surjective_of_eventually_surjective
  ...
  obtain ⟨a, ha⟩ := (Ultrafilter.eventually_exists_of_finite _).mp this
