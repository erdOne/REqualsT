...
variable [TopologicalSpace Λ] [TopologicalRing Λ]
...
noncomputable
instance IsPatchingSystem.isModuleQuotient [PatchingAlgebra.smulData Λ R M] (α : OpenIdeals Λ) :
...
instance [PatchingAlgebra.smulData Λ R M] : IsScalarTower Λ (PatchingAlgebra R F) (PatchingModule Λ M F) := .of_algebraMap_smul <| by
  intro r m
...
variable [IsLocalRing R₀] [IsNoetherianRing R₀]
  [TopologicalSpace R₀] [TopologicalRing R₀] [CompactSpace R₀] [IsAdicTopology R₀]
...
variable [TopologicalSpace Rₒₒ] [CompactSpace Rₒₒ] [TopologicalRing Rₒₒ] [Algebra.TopologicallyFG ℤ Rₒₒ]
...
lemma Ultrafilter.eventually_eventually_eq_of_finite
  ...
  obtain ⟨a, ha⟩ : ∃ a, ∀ᶠ i in F, f i = a :=
    (Ultrafilter.eventually_exists_of_finite F).mp (by simp)
...
lemma support_eq_top
  ...
  rw [← Submodule.map_algebraMap_smul, ← Module.support_eq_zeroLocus,
    ...
    Module.annihilator_eq_bot.mpr inferInstance, ← Module.comap_annihilator (S := R₀),
