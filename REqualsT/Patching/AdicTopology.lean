...
Reference: Compact Rings, SETH WARNER
...
class IsAdicTopology (R) [CommRing R] [IsLocalRing R]
    [TopologicalSpace R] [TopologicalRing R] : Prop where
...
variable {R} in
omit [TopologicalSpace R] [TopologicalRing R] [IsAdicTopology R] in
...
lemma compactSpace_of_finite_residueField [IsNoetherianRing R] [Finite (ResidueField R)]
  ...
  have : Topology.IsClosedEmbedding f := by
    refine ⟨⟨?_, ?_⟩, ?_⟩
    · refine TopologicalAddGroup.isInducing_of_nhds_zero f.toAddMonoidHom ?_
